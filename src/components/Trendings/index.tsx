...
const promise = await fetch('https://api.themoviedb.org/3/movie/popular?language=en-US&page=1', options)
  ...
  <>
    <h1 className="
        p-5 
        font-mono italic 
        bg-gradient-to-r from-purple-500 to-orange-400"> Trending today everywhere </h1>
      
      <div className="grid grid-cols-3 gap-10">
        {data.map((content) => {
          return(
            <Link href={""} key={content.id.toString()} className="p-5 border-4 border-red-500 backdrop-blur-sm">
              <Image 
