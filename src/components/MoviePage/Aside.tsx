  ...
  return (
    <aside className="p-5 pb-30 min-w-[25%] h-max flex flex-col items-start justify-start text-white bg-gradient-to-b from-gray-600 to-black rounded-xl shadow-lg shadow-black">
      ...
      <div className="w-10/12 pt-5 flex items-baseline gap-2 flex-wrap italic text-sm font-mono font-extralight">
        {genre_ids.map((genderId: number) => (
          <Link 
            key={genderId} 
            className="p-1 bg-gradient-to-b from-purple-600 to-orange-600 rounded-full ease-out hover:text-black hover:from-purple-300 hover:to-orange-300" 
            href={{ pathname: "/GendersPage", query: { gender: gendersIds[genderId] } }}
          > 
            {gendersIds[genderId]} 
          </Link>
        ))}
